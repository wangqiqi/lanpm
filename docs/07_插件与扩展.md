...
| 不用甘特/日历/白板/脑图底栏 | **导航偏好** `hiddenViews` / `hiddenContributedRoutes`（默认已藏 gantt·calendar·files·whiteboard·mindmap） | 删掉任务数据 |
...
- 底栏 **主轴** Tab 由 **`VIEW_TABS`**（`paths.ts`）+ **`isViewAllowedForGroup`**（`tabRules.ts`）+ **`navPreferences`** 驱动；默认可见 **chat / board / tree**；`gantt` / `calendar` / `files` / `whiteboard` 与贡献 `mindmap` **默认隐藏**（时间透镜 SPRINT-45；附件 v1.96.0）。附件主入口仍为聊天 **`ChatCollaborationDrawer`**。  
