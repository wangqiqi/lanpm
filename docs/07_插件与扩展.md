...
- 同一 Slot 在文本/语音模式 **双挂载**（`PluginZoneHost` + `PluginGroupSlot`）— 须按输入模式单路径接线。  
...
| 写 | 聊天 | `chat.sendText` · `chat.sendTaskRef` | 中高 · **`sendTaskRef` v0.2 ✅** · **`sendText` v0.5 人审 ✅** |
...
| `verify:ops-gateway-spike` | SPIKE HTTP 网关 · path 白名单 |
| `verify:ops-agent` | Ops MVP · agent CLI · P2P 入站/出站 · 斜杠命令 |
...
> **SSOT**：插件与导航扩展相关未立项 Sprint。`.cursorGrowth/plan.md` 与本节对齐。
...
**建议顺序**：会议体验产品化 · `lanpm.ops` 等可购插件。**勿**立项应用商店。
...
### 12.4 会议实现（录制 + 日程 MVP · v1.81.0）

Goal（已完成）：媒体 Slot/capability + `lanpm.meeting` + 聊天 `MeetingToolbar` · mesh/LiveKit · 许可闸 · Profile 深链 · **本地录制** · **群级日程 + 本机提醒**。

验收：`verify:meeting-ux` · `verify:meeting-recording` · `verify:meeting-schedule` · `verify:meeting-plugin` · Spec 本地 `.cursorGrowth/specs/003-meeting-ux-next/`。

**Out of scope（仍暂缓）**：云端录制 · 日程 Sync · 完整日历视图。
