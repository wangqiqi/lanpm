...
| `/status` | CPU（核数 + load）/ 内存（% + used/total）/ 磁盘（Agent `root` 分区 % + used/total）/ **GPU（可选，`nvidia-smi` 首卡；无则省略）** | Agent | ✅ v1.95.0 |
...
*下编与代码真源对齐：Phase 1 已交付 v1.94.0–v1.94.1；Phase 2+ 见 §8.2 与 ROADMAP §5。*
