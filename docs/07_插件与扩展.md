...
| **协作附件** | 白板、文件、脑图 | 围绕项目上下文；**文件可藏 Tab**；白板/脑图**不是**底栏 Tab |
| **免费扩展模块** | `lanpm.example`、`lanpm.backup`（群备份） | `pricing: free` |
| **可购扩展** | `lanpm.meeting` · `lanpm.mindmap` · `lanpm.ops` · `lanpm.schedule` · `lanpm.agile` · `lanpm.weekly` | `pricing: paid`；离线许可证闸 ✅ v1.75–1.76（`license.feature`） |
...
| **协作附件** | 文件、白板、脑图 | 默认 **聊天 Composer 抽屉** 为主入口；文件可开底栏 Tab；**白板/脑图永不进底栏**（`NEVER_BOTTOM_NAV_*`） |
...
| 不用甘特/日历/文件底栏 | **导航偏好** `hiddenViews`（默认已藏 gantt·calendar·files） | 删掉任务数据 |
| 白板/脑图不进底栏 | 硬锁（非导航开关）；仍走 Composer 抽屉与深链 | 当 Tab 卖画布 |
...
当前默认启用：`lanpm.ai-assistant` · `lanpm.mindmap` · `lanpm.backup`（`enabledDefaults.ts`）。
...
| `profile.data.backup` | Profile 数据存储 · 单群备份区 | ✅ `DataStoragePanel`（`lanpm.backup` 默认开） |
...
- 底栏 **主轴** Tab 由 **`VIEW_TABS`**（`paths.ts`）+ **`isViewAllowedForGroup`**（`tabRules.ts`）+ **`navPreferences`** 驱动；默认可见 **chat / board / tree**；`gantt` / `calendar` / `files` **默认隐藏**（时间透镜 SPRINT-45；附件 v1.96.0）。**`whiteboard` 与贡献 `mindmap` 永不进底栏**（`NEVER_BOTTOM_NAV_*`，Profile 无开关）。附件主入口仍为聊天 **`ChatCollaborationDrawer`**。  
...
```
BottomNav 最终列表 =
  核心：群类型允许 ∩ 主轴约束 ∩ !hiddenViews ∩ !NEVER_BOTTOM_NAV_VIEWS → 按 order
  + 插件贡献：groupTypes ∩ !hiddenContributedRoutes ∩ !NEVER_BOTTOM_NAV_CONTRIBUTED_ROUTES → 按 contributedOrder（核心之后）
```
...
| `files` | 是 | 附件（职能群仍受群类型约束） |
| `whiteboard` | **否（永不进底栏）** | 聊天/任务工具；深链全屏仍可用 |
| 插件贡献 Tab | 是（**`mindmap` 除外**） | `mindmap` 永不进底栏；其余段内排序 · v1.77.0 |

### 3.7.4 设置入口（产品）

- Profile → **「导航与视图」**：核心 Tab（不含白板）+ 已发现插件贡献 Tab（不含脑图）；拖拽排序 + 开关。  
...
| `lanpm.mindmap` | `mindmap.toolbar` · `contributions.views` | paid | 否 | `MindmapView`（mind-elixir）/ `MindmapStub` |
...
| `lanpm.schedule` | `gantt.toolbar` | paid | 否 | `ScheduleStub`（关键路径 + 冻结基线 + 指派重叠） |
| `lanpm.agile` | `board.toolbar` · `board.card.footer` | paid | 否 | `AgileStub`（故事点 + 燃尽 + 列 WIP + 迭代容器 + 速度图） |
| `lanpm.weekly` | `topbar.menu` | paid | 否 | `WeeklyStub`（驾驶舱导出走 IPC 闸） |
| `lanpm.backup` | `profile.data.backup` | free | 是 | Host `DataStoragePanel` + `BackupStub` |
...
| 会议体验深化 | 工具条/许可闸 ✅ v1.78.4；录制/日程 MVP ✅ v1.81.0；产品化（i18n · 通知深链 · join CTA）✅ v1.89.0；**日程可编辑** ✅ Unreleased |
...
**建议顺序**：见 `.cursorGrowth/plan.md` 候选表。Ops Phase 2 与会议产品化基线 **已交付**。**勿**立项应用商店。
...
#### 本机 / 内网 LiveKit 旁路（最短步骤）

不新造 SFU。用官方 `livekit-server` 镜像在本机或内网主机起旁路（compose：`plugins/lanpm.meeting/deploy/docker-compose.yml`）。

1. `cd plugins/lanpm.meeting/deploy`，复制 `.env.example` → `.env`，填 `LIVEKIT_API_KEY` / `LIVEKIT_API_SECRET`（Secret ≥ 32 字符）。
2. `livekit.yaml` 的 `keys` 与 `.env` 一致。
3. `docker compose up -d`。
4. Profile → 会议旁路填写 URL：本机 `ws://127.0.0.1:7880`，内网 `ws://<旁路主机>:7880`（已配 TLS 则用 `wss://`），以及同一套 Key/Secret。

细则见 `plugins/lanpm.meeting/README.md`。默认只连自托管旁路，禁止把公有 SFU 当默认。
...
**Out of scope（仍暂缓）**：云端录制 · 日程 Sync · 完整日历视图。日程 **edit** 已交付（`updateSchedule` · `verify:meeting-schedule`）。

### 12.5 form-js · 市场 · 思维导图（已交付指针）

- **form-js**：~~`verify:formjs-plugin`~~ **已移除** v1.100.1（`lanpm.formjs` POC 下线）
- **分发**：侧载+`license.feature` ✅ v1.75–1.76；详见本篇 **§13** 起
- **思维导图**：`lanpm.mindmap` · Layer C · ✅ v1.74.0；**v1.102.0** 起每群多张独立文档 + 导出/分享矩阵（见 `docs/04` §2）
...
| form-js-real | v1.73.0（**已移除** v1.100.1） | — |
...
适用：安装包已含 `lanpm.meeting` · `lanpm.mindmap` 等可购插件，客户只需授权。
...
| `npm run verify:plugin-distribution-playbook` | 离线分发 playbook 聚合 · 手验模板 · [05 §1.2.10](./05_测试与联调发布.md#1210-插件离线分发回归侧载--许可--非应用商店-ci) |
