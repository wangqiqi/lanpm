...
| **验收** | `npm run verify:ops-agent` · `npm run verify:ops-p2` · `npm run verify:ops-gateway-spike` |
...
| **Phase 2（剩余）** | Ops 助手 Bot · 出站 watch · 任务挂包深化 | 后置 |
| **Phase 3** | Gateway HTTP + Web Terminal（可选） | 后置 |
| **Phase 4** | L2 双向同步（若确有需求） | 单独评估 |

**验收锚点**：`verify:ops-gateway-spike` · `verify:ops-agent` · `verify:ops-p2`（Phase 2 闭环）。
