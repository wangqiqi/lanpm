...
### 6.1 内置命令（Phase 1 · 已交付）

| 命令 | 作用 | 执行方 | 状态 |
|------|------|--------|------|
| `/help` | 列出本群可用命令与 `@机器` 用法 | Agent | ✅ v1.94.0 |
| `/logs [app\|nginx\|…]` | 取最近日志，以**文件消息**回群 | 目标 Agent | ✅ v1.94.0 |
| `/status` | CPU（核数 + load）/ 内存（% + used/total）/ 磁盘（Agent `root` 分区 % + used/total · `statusSnapshot.ts`）；**不含 GPU**（→ Phase 2） | Agent | ✅ v1.94.0–v1.94.1 |
| `/deploy [file]` | 将最近安装包或指定附件写入 `inboundDir` | Agent | ✅ v1.94.0 |
| `/disk` `/ps` | 常见巡检快照（只读） | Agent | ✅ 代码已落地 |
| `/tail <path>` | 日志尾部若干行（长度上限 · 路径白名单） | Agent | ✅ 代码已落地 |
...
| 层 | Phase 1 |
|----|---------|
| L1 斜杠命令 | ✅ 已交付（`/help` `/logs` `/status` `/deploy` · `/disk` `/ps` `/tail`） |
...
| **斜杠命令解析** | 群 composer · `ops_command` / `ops_command_result` · `commandExecutor.ts` |
| **`lanpm.ops` 插件** | `plugins/lanpm.ops/` · Slot · 许可闸 · Profile 设置 |
| **机器身份** | `deviceKind: machine` · 显示名 · 上下线系统消息 |
| **命令审计** | `ops-command-audit.json` · Profile 只读表格 |
| **验收** | `npm run verify:ops-agent` · `npm run verify:ops-gateway-spike` |
...
| **出站 watch** | 目录变更自动推送到群（可选） |
| **Gateway HTTP/WS** | 浏览器访问文件列表/Terminal |
| **Ops 助手 Bot** | 群成员身份 · L2 短答 · 命令补全 |
| **「在助手中分析」** | 日志/状态气泡 → 打开助手并注入上下文（L3） |
| **GPU 指标** | `/status` 扩展 |
...
| **Phase 2** | Ops 助手、出站 watch、L3 助手分析入口、任务挂包深化 | 进行中 / 后置 |
| **Phase 3** | Gateway HTTP + Web Terminal（可选） | 后置 |
| **Phase 4** | L2 双向同步（若确有需求） | 单独评估 |

**验收锚点**：`verify:ops-gateway-spike` · `verify:ops-agent`（含 `statusSnapshot.ts` 磁盘字段）。
