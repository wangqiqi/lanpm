...
- 任务协同编辑（描述、标题并发编辑）：使用 Yjs `Y.Doc`，文档 ID = `task:{groupId}`（每项目群组单文档，见 §10）。
...
| 看板列数据 | 仅 **根任务**（`parentTaskId` 为空）进入看板四列 |
| 同步 | **当前仅本机 SQLite**；`task_patch` / Yjs（§10）待 M6 前迭代 |

详见 [07_M3_看板与任务树实现说明.md](./07_M3_看板与任务树实现说明.md)。
...
3. `Task` 基础 CRUD + 看板 status — **M3 已完成（本机）**，见 [07](./07_M3_看板与任务树实现说明.md)
...
## 12. 运行时默认策略（已定稿，M6 前可微调参数）

| 项 | 默认策略 |
|----|----------|
| 离线消息保留窗口 | **7 天**；配置键 `sync_meta.message_ttl_days` |
...
| `docs/05_交互与UI约定.md` | 布局、主题、组件约定 |
| `docs/06_测试与联调手册.md` | Stub/真网/性能测试步骤 |
