...
> **关联**：[06_ROADMAP](./06_ROADMAP.md) §2–§3（市场 · 会议插件）· [飞鸽飞秋](./飞鸽飞秋.md) §7（收费边界）· [04_交互与UI约定](./04_交互与UI约定.md) §2（导航）· [02_技术实现建议](./02_技术实现建议.md) §15.3（AI Capability 扩展）  
...
| `contributes.*`（views / menus / commands） | `plugin.json` → **`slots[]`** + **`contributions.views[]`**（Layer C · P2） |
...
收费边界详见 [飞鸽飞秋](./飞鸽飞秋.md) §7.0：**主路径高频能力必须免费**；小众/重资源可购。
...
| **Layer C** | **贡献新 Tab**（P2） | 整页插件视图（思维导图、报表…） | `contributions.views[]` | 📋 未实现 |
...
| 5 | **预留 Layer C** | 宿主不假设底栏只有 7 项；`VIEW_TABS` 未来可合并 `contributions.views`（§3.6） |
...
|-----------|------------------|-------------------|----------|
| `chat` | `ChatView` | `chat.toolbar.media`（toolbar）· `chat.composer.action`（composer）· `chat.message.action`（context） | 📋 均未接 |
| `board` | `BoardView` | `board.toolbar`（toolbar）· `board.card.footer`（card）· `task.detail.section`（detail · Modal） | 📋 均未接 |
| `tree` | `TreeView` | `tree.toolbar`（toolbar）· `task.detail.section`（detail） | ⚠️ 详情仅面板已接 |
| `gantt` | `GanttView` | `gantt.toolbar`（toolbar）· `gantt.bar.context`（context） | 📋 均未接 |
| `calendar` | `CalendarView` | `calendar.toolbar`（toolbar）· `calendar.event.action`（context） | 📋 均未接 |
| `whiteboard` | `WhiteboardView` | `whiteboard.toolbar`（toolbar） | 📋 均未接 |
| `files` | `FilesView` | `files.toolbar`（toolbar）· `files.preview.action`（preview） | 📋 均未接 |
...
|------|------|-----------------|------|
| 读 | 任务 | `task.getChecklist` | 低 |
| 读 | 聊天 | `chat.listMessages` | 低 |
| 读 | 成员 | `member.list` | 低 |
| 写 | 任务 | `task.patch` · `task.create` | 中 · 建议人审 |
| 写 | 聊天 | `chat.sendText` · `chat.sendTaskRef` | 中高 |
| 写 | 文件 | `file.upload` | 中高 |
...
| **v0.2** | 聊天/看板读 + `chat.sendTaskRef`；`chat.toolbar.media` | 📋 会议 Sprint |
...
| **M4** | `contributions.views`（Layer C）· 命令面板 · `commands[]` | 📋 P2 |
| **M5** | **插件市场** · 侧载 · 签名 · 离线许可证 | 📋 P2 · M8 |

### 8.2 插件市场（可做 · P2）

对齐 [飞鸽飞秋](./飞鸽飞秋.md) §11.8 · [06_ROADMAP](./06_ROADMAP.md) §2。
...
阶段：P1 侧载 + 授权 POC → P2 商店 UI + 私有源 → 受控第三方上架。
...
| `verify:nav-preferences` | **待补** · Layer A |
| `verify:view-slot-hosts` | **待补** · §3.5 各 Tab Slot 锚点 |
...
- **form-js**：`FormJsPoc` → 真库；Slot 不变；`verify:formjs-plugin`  
- **市场**：P1 侧载+`license.feature`；P2 商店+私有源；立项前 SPIKE 包格式/签名  
- **思维导图**：可购 · 任务关联；优先 **Layer C** `contributions.views`（§3.6），Tab 内用 `mindmap.toolbar`；排在 ViewHost + 会议之后  

### 12.6 Layer C · 贡献新 Tab（详纲 · P2）
...
| 验收 | `verify:contributions-views`（规划） |
...
| PLUGIN-FORMJS | v1.22 | `verify:plugin-loader` |
