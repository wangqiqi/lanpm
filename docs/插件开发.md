...
**现状**：七核心 Tab + 任务详情 + 三全局 Slot 已接线；残余见 §8.3（命令 invoke 真执行、会议体验深化）。
...
| ~~多视图 Slot 未接~~ | ✅ §3.5.3；全局 Slot ✅ v1.78.2 |
...
**残余**：`chat.composer.action` 等 zone slot 查漏（见 slot-wiring-residual 候选）。
