...
| `contributes.*`（views / menus / commands） | `plugin.json` → **`slots[]`** + **`contributions.views[]`**（Layer C · ✅ v1.74.0）+ **`commands[]`**（命令面板 · ✅ v1.78.0） |
| Extension API | `window.lanpm.plugin.invokeCapability`（preload 暴露） |
| `activate()` 跑扩展代码 | **builtin registry** 映射 React 组件（见 §3.2） |
| Command Palette | **✅ POC** Ctrl/Cmd+K（v1.78.0 · Host invoke stub） |
| Marketplace | **商店未做**（侧载+离线许可 ✅ · ROADMAP P2 · 见 §8.2） |
...
| 聊天加会议 | `chat.toolbar.media`（✅ 宿主已接 · 会议体验深化 📋） | 群顶堆满图标 |
...
| 层 | 名称 | 解决什么 | 典型能力 | 状态 |
|----|------|----------|----------|------|
| **Layer A** | **导航偏好** | 我用哪些核心 Tab、什么顺序 | `hiddenViews` · `order` · 每群覆盖（可选） | ✅ 全局 MVP v1.77.0；每群覆盖 📋 P2 |
...
| `chat.toolbar.media` | 聊天输入区媒体工具 | ✅ 宿主已接 · 会议体验深化 📋 |
...
**契约**：`ViewPluginContext` + `PluginZoneHost` / `PluginSlotHost`；`PluginTaskSlot` / `PluginGroupSlot` 为薄封装。地图 SSOT：`src/renderer/src/plugin/viewSlotMap.ts`。
...
**现状**：七核心 Tab + 任务详情多入口已按 §3.5.3 接线；残余见 §8.3（`topbar.menu` / `profile.tab`、命令 invoke 真执行、会议体验深化）。
...
## 3.7 导航与 Tab 定制 · Layer A（✅ 全局 MVP · v1.77.0）
...
| **每群覆盖**（P2 可选） | 「本群只要聊天+看板」 | 群 meta / SQLite · **未做** |
...
**代码现状**：`NavPreferences` MVP ✅（核心 v1.x · 插件贡献 **v1.77.0**）。
...
| **v0.3** | `task.patch` · `board.*` 写白名单 | 📋 |
| **v0.4** | `license.feature` · 侧载格式 | ✅ v1.75–1.76（商店 UI 仍 📋） |
...
| `slots` | 条件 | `PluginSlotId[]`；可与 `contributions.views` / `commands` 任一非空组合（三者不能全空） |
| `capabilities` | ✅ | 可为 `[]` |
| `pricing` | ✅ | `free` \| `paid` |
...
- IPC：`plugin:listCommands` · `plugin:invokeCommand`（invoke 为 Host stub，无插件沙箱执行）
...
- **会议**：聊天输入旁 → `chat.toolbar.media`（stub/mesh/LiveKit 路径已交付；体验深化 📋）  
...
| **M3** | API v0.3 写白名单（`task.patch` · `board.*`） | 📋 |
| **M4** | `contributions.views`（Layer C）· 命令面板 · `commands[]` | Layer C ✅ · 命令面板 POC ✅ v1.78.0 |
| **M5** | **插件市场** · 侧载 · 签名 · 离线许可证 | 侧载+许可 ✅ v1.75–1.76 · 商店 📋 P2 |
...
| ~~无 `NavPreferences`~~ | ✅ 全局 MVP v1.77.0；**每群覆盖**仍 📋 |
| ~~多视图 Slot 未接~~ | ✅ §3.5.3；残余 `topbar.menu` · `group.tab.overflow` · `profile.tab` |
| 写 capability 不足 | API v0.2 已扩读+`chat.sendTaskRef`；**`task.patch` / `board.*`** 仍 📋 v0.3 |
| 命令 `invoke` 为 Host stub | 面板 POC ✅；真 handler 表 📋 |
| 会议体验深化 | Host/stub/mesh/LiveKit 路径 ✅；产品化 UX 📋 |
| 须合入 registry | 第三方不能 drop-in；商店 📋 |
...
| 维度 | 结论 |
|------|------|
| **是否「最灵活」终态？** | **否** — 底座到 **M2/M4 POC**；商店与写 API 未完 |
| **插件启停** | ✅ Profile 开关 + 持久化 + Slot 刷新（已接线处） |
| **Tab 启停/排序** | ✅ Layer A 全局（v1.77.0）；每群覆盖 ❌ |
...
| **ViewHost / Slot 残余加固** | `topbar.menu` · `profile.tab` · 契约收口 · 锚点查漏 | P1 |
| **会议体验下一刀** | UX / LiveKit 产品化（Host stub 已有） | **P1** |
| **命令 invoke 真执行** | Host handler 表（面板 POC ✅ v1.78.0） | P1 |
| **Extension API v0.3** | `task.patch` · `board.*` 白名单 | P1 |
| **每群导航覆盖** | Layer A P2 | P2 |
| **插件市场** | 商店 UI · 私有源（侧载+许可 ✅） | **P2 · M8** |

**建议顺序**：会议体验 / Slot 残余 ∥ 命令 invoke → API v0.3 → 每群导航 → 市场 SPIKE/MVP。
...
**残余**：跨群 `topbar.menu` / `profile.tab` 等未接线 Slot。

### 12.4 会议实现（Host 已交付 · 体验深化待立）

Goal（已完成部分）：媒体 Slot/capability + `lanpm.meeting` + 聊天 → `chat.toolbar.media` · mesh/LiveKit 路径。

下一刀：产品化 UX；详见 SPIKE archive · `verify:meeting-plugin` 等。

### 12.5 form-js · 市场 · 思维导图（已交付指针）

- **form-js**：`verify:formjs-plugin` ✅ v1.73.0
- **市场**：侧载+`license.feature` ✅ v1.75–1.76；P2 商店+私有源  
