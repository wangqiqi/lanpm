...
| Marketplace | **商店暂不做**（侧载+离线许可 ✅ · 等盈利再议 · 见 §8.2） |
...
| — | `license.feature` · 侧载格式（历史交付，勿再称「v0.4」） | ✅ v1.75–1.76（商店 UI 仍 📋） |
...
- **v1.83.0+**：聊天页经 `ChatPluginMenusProvider` 会话级拉取一次 `listMenus`（`pluginMenusCache`）；`MessageBubble` 用 `useChatPluginMenuItems`，**勿**在气泡内直接 `usePluginMenus`
- IPC：`plugin:listMenus`；点击 → `plugin:invokeCommand` → `applyCommandAction`
...
| **M5** | **插件市场** · 侧载 · 签名 · 离线许可证 | 侧载+许可 ✅ v1.75–1.76 · **商店暂不做**（等盈利） |

### 8.2 插件市场（P1 POC ✅ · 商店暂缓）

对齐 [06_ROADMAP](./06_ROADMAP.md) §2 · 本节 §12（Electron 生存 · 市场）。

| 形态 | 说明 |
|------|------|
| 公开市场 | 浏览、购买、更新、签名校验 — **暂不做** |
| 内网私有源 | **不依赖公网** · 离线许可证 · 信创刚需 — 商店层暂缓；侧载已可用 |
| 原则 | 空壳商店无意义；**无盈利 / 无单独投入实力前不立项商店** |
| 拍板 | **2026-07-31**：商店 UI / 私有源运营 / 第三方上架均 **暂缓** |

阶段：**P1 侧载 + 授权 POC ✅ v1.75.0** → ~~P2 商店 UI + 私有源~~ **冻结** → 盈利后再议。
...
| ~~**插件市场**~~ | 商店 UI · 私有源（侧载+许可 ✅） | **暂缓** · 等盈利 |

**建议顺序**：会议体验产品化。**勿**把商店排进下一刀。
...
验收：`verify:meeting-ux` · `verify:meeting-recording` · `verify:meeting-schedule` · `verify:meeting-plugin` · Spec `docs/specs/003-meeting-ux-next/`。
...
- **市场**：侧载+`license.feature` ✅ v1.75–1.76；商店+私有源 **暂缓**（等盈利）
