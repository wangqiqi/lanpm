...
**现状**：七核心 Tab + 任务详情 + 三全局 Slot 已接线；命令 invoke 真执行 ✅ v1.78.3；会议工具条 UX ✅ v1.78.4；残余见 §8.3（会议录制等深化）。
...
| 会议体验深化 | 工具条/许可闸 ✅ v1.78.4；录制/日程等产品化 📋 |
...
| **会议体验下一刀** | 录制/日程等产品化（工具条 ✅ v1.78.4） | P2 |
...
### 12.4 会议实现（Host 已交付 · 工具条 v1.78.4）

Goal（已完成）：媒体 Slot/capability + `lanpm.meeting` + 聊天 `MeetingToolbar` · mesh/LiveKit 路径 · 许可闸 · Profile 深链。

下一刀：录制/日程等产品化；详见 `verify:meeting-ux` · `verify:meeting-plugin`。
