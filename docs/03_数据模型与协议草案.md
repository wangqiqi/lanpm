...
**群标签字典（SPRINT-GROUP-TAG-DICT）**：表 `group_tag_meta`；报文 `group_tag_patch` + `GroupTagPatchPayload`（`src/shared/task/groupTagMeta.ts`）；LWW by `updatedAt`；芯片色优先字典，否则 `tagColorHash`。**任务 `tags` 强制 ⊆ 字典**（`filterTagsToGroupDict`）：空字典只能空标签；历史孤儿可读，保存/同步写入时丢弃。
