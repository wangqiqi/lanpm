...
**Awareness（焦点 Presence，SPRINT-TASK-AWARENESS）**：挂同一群级 `Y.Doc` 的 Yjs Awareness（`y-protocols`）；报文 `task_awareness` + `TaskAwarenessPayload`（`src/shared/task/taskAwareness.ts`）。本地态字段：`userId` · `displayName` · `focusedTaskId?` · `view: board|tree|null`。**短暂态、不落库**；不做文本 caret。匿名群不广播（与 `task_crdt` 一致）。
