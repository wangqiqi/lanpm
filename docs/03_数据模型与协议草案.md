...
**群标签字典（SPRINT-GROUP-TAG-DICT）**：表 `group_tag_meta`；报文 `group_tag_patch` + `GroupTagPatchPayload`（`src/shared/task/groupTagMeta.ts`）；LWW by `updatedAt`；芯片色优先字典，否则 `tagColorHash`。
