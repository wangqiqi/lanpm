...
type SyncMessageType =
  ...
  | 'file_meta'      // 文件索引
  | 'file_chunk'     // 文件分片
  | 'member_event'   // 入群/退群等（RC：**协议保留**；main 无 handler；`publish` 拒绝）
  | 'group_key_rotate'; // 群组密钥轮换通知
```

> **RC 发送守卫**：`task_crdt` / `member_event` 见 `src/shared/network/unimplementedSync.ts` 与 [06](./06_验收与里程碑计划.md) §2.3；实现 handler 前禁止出站。
...
| 维度 | 策略 | 配置 / 代码 |
|------|------|-------------|
| **P2P 补拉窗口** | 固定 **7 天**；仅过滤入站 `chat_sync_batch`，不删本机旧消息 | `SYNC_WINDOW_DAYS`、`offlineSyncService` |
