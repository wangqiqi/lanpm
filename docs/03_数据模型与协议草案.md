...
> 数据模型与协议草案。索引见 [00_文档导航.md](./00_文档导航.md)。
> 对齐文档：`docs/00_文档导航.md`、`docs/01_产品需求文档.md`、`docs/02_技术实现建议.md`、`docs/06_ROADMAP.md`  
...
type SyncMessageType =
  ...
  | 'task_crdt'      // Yjs 更新帧（TASK-157 可 publish；handler TASK-159）
  | 'task_crdt_sync_request' // Yjs 离线补拉请求（state vector，TASK-161）
  | 'task_crdt_sync_batch'   // Yjs 离线补拉 diff
  | 'task_awareness' // Yjs Awareness 焦点 Presence（TASK-177+；不落库）
  | 'whiteboard_crdt'
  | 'whiteboard_crdt_sync_request'
  | 'whiteboard_crdt_sync_batch'
  | 'whiteboard_awareness'
  | 'mindmap_crdt'      // 脑图 Yjs 更新（SPRINT-26；docId = mindmap:{docId}）
  | 'mindmap_crdt_sync_request'
  | 'mindmap_crdt_sync_batch'
  | 'mindmap_awareness'
  | 'group_tag_patch' // 群标签字典色 LWW（TASK-189+）
  | 'group_tag_sync_request' // 群标签字典离线补拉请求（TASK-4101）
  | 'group_tag_sync_batch'   // 群标签字典离线补拉批次
  | 'file_meta'      // 文件索引
  | 'file_meta_sync_request' // 文件库索引离线补拉请求（TASK-4801）
  | 'file_meta_sync_batch'   // 文件库索引离线补拉批次
  | 'file_pull_request' // 文件拉取请求（可带 fromOffset 续传，TASK-164）
  | 'file_chunk'     // 文件分片
  ...
  | 'ops_command'        // 运维斜杠命令（群 → Agent，v1.94.0）
  | 'ops_command_result' // 运维命令回执（Agent → 群，v1.94.0）
  | 'ops_inbound';       // 聊天文件入站落盘（群 → Agent，v1.94.0）
```

> **发送守卫**：`UNIMPLEMENTED_SYNC_TYPES` 已空（`task_crdt` 于 TASK-157 解禁；`member_event` 于 TASK-146）。实时 apply 见 TASK-159。
...
interface GroupMember {
  ...
  /** 运维机器成员（v1.94.0 · lanpm-agent） */
  deviceKind?: 'human' | 'machine';
  deviceId?: string;        // machine 时用于 @提及与 ops_command 路由
}
...
interface Task {
  ...
  /** 到期日 / 区间结束（日历以 endDate 为主；有 startDate 则跨日） */
  endDate?: string;
  /** 依赖：存储为有向边 */
...
- 任务协同编辑（描述、标题等全量字段）：Yjs `Y.Doc`，文档 ID = `task:{groupId}`（每项目群组单文档，见 §10）；实时帧 `task_crdt`（与 `task_patch` 共存，见 SPRINT-TASK-CRDT）。
...
| 同步 | **实时** `task_patch` / `task_dep_patch`（LWW）+ `task_crdt`（Yjs，TASK-157+）；**离线补拉** `task_sync_request` → `task_sync_batch`（7 天窗，含软删）；Yjs 离线补拉 `task_crdt_sync_request` → `_batch`（state vector） |
...
**离线补拉（A6 · TASK-4801）**：重连时 `file_meta_sync_request`（`sinceUpdatedAt` + `minUpdatedAt`）→ 对端 `file_meta_sync_batch`（非书签索引，按 `updatedAt` 分页 `hasMore`）。收端 LWW upsert 为 `remote-pending:`，**不**自动 `file_pull` 字节。实时仍走单条 `file_meta`。
...
interface AiTaskAuditPayload {
  ...
  startDate?: string;
  endDate?: string;
  descriptionSummary?: string;  // 可选截断 ≤80 字
...
| 规则 | 说明 |
|------|------|
| 会话仅本地 | `ai_threads` / `ai_messages` 按本机 `userId`；**不参与**群组 P2P 同步 |
| 可发到群聊 | 用户主动 `shareToChat`；消息 `meta.source: 'ai-assistant'` |
| `#` 任务引用 | 与群聊共用 `@shared/chat/taskRefs`；外呼时解析为脱敏 `AiTaskAuditPayload` |
| 助手内置免费 | `lanpm.ai-assistant` builtin · `pricing: free` |

技术实现见 [02](./02_技术实现建议.md) §15。

### 3.7 AI 本地表（不进 P2P）

| 表 | 用途 |
|----|------|
| `ai_config` | 厂商 · 加密 `api_key_enc` · baseUrl · model · enabled · patrol_* |
| `ai_threads` | 本机会话；可选 `group_id` 标签（筛选用，不同步） |
| `ai_messages` | 线程消息；`role` user/assistant/system |
| `ai_patrol_runs` | 定时巡检摘要与状态 |
| `ai_pipeline_runs` | L3 流水线运行记录 · `steps_json`（含 `pendingConfirm`） |

`AI_IPC` 通道清单见 `src/shared/ai/channels.ts` · [02](./02_技术实现建议.md) §15.3。
...
/** 文件库索引离线补拉（TASK-4801）；7 天窗，按 updatedAt 分页 */
interface FileMetaSyncRequestPayload {
  sinceUpdatedAt: string;       // 排他下界；空串=从 epoch
  minUpdatedAt: string;         // 7 天 cutoff ISO8601
}

interface FileMetaSyncBatchPayload {
  files: FileMeta[];            // 无本机 storagePath；收端记 remote-pending
  hasMore?: boolean;
}

/** 文件拉取请求（TASK-164 / TASK-3401）；fromOffset 缺省=0 全量 */
interface FilePullRequestPayload {
  fileId: string;
  groupId: string;
  fromOffset?: number;        // 已收字节，续传起点
  chunkEncoding?: 'base64' | 'binary'; // 缺省=base64（旧客户端）；binary=密封明文 framed 片
}

/** JSON 路径（旧客户端 / 未声明 encoding） */
...
/**
 * binary 路径：仍 type=`file_chunk`；密封明文为魔数 `LPM1` + uint32 头长 + 头 JSON + 原始片。
 * 仅当 pull 请求 `chunkEncoding: 'binary'` 时发送。
 * TCP：双方 handshake `envBin` 时 `kind: envelope_bin`（4B 头长 + JSON 元数据 + 密文 raw）；
 * 旧对端无 `envBin` 时仍 `kind: envelope` + payload.`__enc` Base64。
 */
...
/** Yjs 实时更新帧（TASK-157）；docId = `task:{groupId}` */
interface TaskCrdtPayload {
  docId: string;              // `task:{groupId}`
  updateBase64: string;       // Y.encodeStateAsUpdate / update 的 base64
}

/** Yjs 离线补拉（TASK-161） */
interface TaskCrdtSyncRequestPayload {
  docId: string;
  stateVectorBase64: string;  // 空串=请求全量
}

interface TaskCrdtSyncBatchPayload {
  docId: string;
  updateBase64: string;       // 相对请求方 SV 的 diff
}
```
...
### 6.5 连接码、发现中继与入群扩展（v1.42+）

> 与 `src/shared/network/pairingTypes.ts` · `discoverRelay.ts` · `groupInvite.ts` · `wireProtocol.ts` 对齐。产品叙事见 [01](./01_产品需求文档.md) §11.4。

#### 6.5.1 UDP 配对报文（端口 `43123`，明文）

与 `{ v:1, kind:'discovery', payload }` 并列，`payload` 字段结构如下（实现用 `kind` 顶层字段，见 `PairingUdpPacket`）：

```typescript
// 发起方周期广播 pairing_offer
interface PairingOfferBody {
  code: string              // "847293"
  pairingId: string
  deviceId: string
  userId: string
...
// 加入方 pairing_lookup（广播或单播）
interface PairingLookupBody {
  code: string
  joinerDeviceId: string
  joinerDisplayName: string
}

// 发起方单播 pairing_found（code 匹配且未过期）
interface PairingFoundBody {
  pairingId: string
  deviceId: string
  userId: string
  displayName: string
...
#### 6.5.2 TCP `pairing_resolve`（端口 `43124`，DH 前明文）

```typescript
type PairingResolveFailReason = 'expired' | 'mismatch' | 'rate_limit'

// WireMessage 片段（见 wireProtocol.ts）
...
#### 6.5.3 发现中继 `discover_relay`（TCP 明文）

```typescript
interface DiscoverRelayPacket {
  v: 1
  kind: 'discover_relay'
  hop: number              // 发送方剩余跳数，初始 ≤2
  viaDeviceId: string
  peers: { deviceId; userId; displayName; host; listenPort }[]
  groups: DiscoverRelayGroupEntry[]  // 含 ownerUserId / ownerDisplayName
...
```typescript
// group_invite_offer / group_invite_lookup / group_invite_found
// 见 src/shared/group/groupInvite.ts — 结构与配对码类似，payload 含 groupId · groupName · groupType
```

#### 6.5.5 入群申请（`SyncEnvelope` 加密业务通道）

```typescript
// envelope.type = 'join_request' | 'join_request_decision'
...
interface JoinRequestDecisionPayload {
  requestId: string
  groupId: string
  applicantUserId: string
  approved: boolean
...
持久化：`group_join_requests` 表（schema v16+）；状态 `pending` \| `approved` \| `rejected`。

#### 6.5.6 Lite mesh 媒体信令（`media_signal`）

```typescript
// envelope.type = 'media_signal'
// 见 src/shared/media/mediaSignal.ts
interface MediaSignalPayload {
  signalId: string
  groupId: string
  kind: 'join' | 'leave' | 'offer' | 'answer' | 'ice-candidate'
  fromUserId: string
  fromDisplayName: string
  toUserId?: string       // WebRTC 定向信令；join/leave 广播
  sdp?: string
  candidate?: string
  at: string
}

interface MediaRoomState {
  groupId: string
  phase: 'idle' | 'active'
...
Main：`mediaSignalService`（`subscribeAll` 收包 · 房间注册表 · `capabilityProxy` 暴露 `media.*`）；投屏源经 `desktopCapturer` Host 代理。Renderer 可用原生 `RTCPeerConnection`（**不**引 npm WebRTC 包）。

#### 6.5.7 U 盘配对文件 `lanpm-peer.json` v1
...
| 参数 | 值 |
|------|-----|
| 码有效期 | 5 min（`PAIRING_TTL_MS`） |
| offer 广播间隔 | 2 s |
| lookup 超时 | 8 s |
| 单加入方失败上限 | 5 次 / 码 |
| lookup 限流 | 12 次 / 分钟 / 加入方 |

#### 6.5.8 运维协作（`ops_*` · v1.94.0–v1.94.1）

> 真源：`src/shared/ops/types.ts` · `src/main/ops/statusSnapshot.ts` · [07_插件与扩展](./07_插件与扩展.md) §33 · `lanpm agent start` / `tools/lanpm-agent/`。

```typescript
// envelope.type = 'ops_command' | 'ops_command_result' | 'ops_inbound'
type OpsCommandName = 'help' | 'logs' | 'status' | 'deploy' | 'disk' | 'ps' | 'tail';

interface OpsCommandPayload {
  requestId: string;
  groupId: string;
  targetDeviceId: string;
  command: OpsCommandName;
  args?: string[];
  fileName?: string;   // deploy 可选内联附件
  dataBase64?: string;
}

interface OpsCommandResultPayload {
  requestId: string;
  groupId: string;
...
- **入站**：桌面端向机器成员发文件 → `ops_inbound` → Agent `inboundDir`（path 白名单）。
- **出站**：`@机器 /logs` 等 → `ops_command` → Agent 读 `outboundPaths` → `ops_command_result` 或群文件消息。
- **`/status`**：CPU · 内存（% + used/total）· Agent `root` 磁盘（`statusSnapshot.ts`）· 可选 GPU（`gpuSnapshot.ts` · `nvidia-smi` 首卡，无则省略）。
- **系统消息**：`ops.machine.online` / `ops.machine.offline`（`messageType: system`）。
- **聊天 meta**：Agent 文本回执可带 `meta.source: 'ops-agent'` · `requestId`。
...
| 方案 | 说明 | 结论 |
|------|------|------|
| 每群单文档 `task:{groupId}` | 一个项目群组一个 Y.Doc，任务为 Y.Map 子结构 | **P0 采用**（SPRINT-TASK-CRDT） |
| 每任务单文档 `task:{taskId}` | 连接数随任务增长 | P1 评估（超大项目再拆） |

**理由**：降低同步开销；看板/树/甘特共享同一 CRDT 源。与 `task_patch` **共存**（双写）：Y.Doc 为协同源，SQLite 仍为结构化查询源。

**协议**：`SyncMessageType.task_crdt` + `TaskCrdtPayload`（`src/shared/task/taskCrdt.ts`）；传输仍走现有 TCP/UDP 信封（非 WebRTC）。

**Awareness（焦点 Presence + 文本 caret）**：挂同一群级 `Y.Doc` 的 Yjs Awareness（`y-protocols`）；报文 `task_awareness` + `TaskAwarenessPayload`（`src/shared/task/taskAwareness.ts`）。本地态字段：`userId` · `displayName` · `focusedTaskId?` · `view: board|tree|null` · 可选 `caret?: { field: 'description', offset }`（UTF-16，与 textarea `selectionStart` 一致）。**短暂态、不落库**。匿名群不广播（与 `task_crdt` 一致）。

**群标签字典（SPRINT-GROUP-TAG-DICT）**：表 `group_tag_meta`；实时报文 `group_tag_patch` + `GroupTagPatchPayload`；离线补拉 `group_tag_sync_request` / `group_tag_sync_batch`（`sinceUpdatedAt` + `minUpdatedAt`，7 天窗，按 `updatedAt` 分页 `hasMore`，LWW upsert；`src/shared/task/groupTagMeta.ts`）。芯片色优先字典，否则 `tagColorHash`。**任务 `tags` 强制 ⊆ 字典**（`filterTagsToGroupDict`）：空字典只能空标签；历史孤儿可读，保存/同步写入时丢弃。
...
-- 脑图文档级 Yjs 快照（SPRINT-26）；JSON 文件仍回写 files.storage_path，CRDT 为真源
...
| **未读角标** | 仅统计保留窗内消息（排除自己发送） | `badgeService.countUnreadMessages` · `GroupTabBadges.chatUnread` |
| **看板角标** | **指派给本地用户**且 `status ∈ {todo, doing}` 的未删除任务；**不是**群内全部 todo | `badgeService.countMineOpenTasks` · `GroupTabBadges.boardMineOpen` · `shared/badge/mineOpen` |
| **看板弱红点** | 仅当 `boardMineOpen === 0`；群内最新 `updatedAt` 在 48h 内且晚于本机 `lastBoardSeenAt`；进看板后清除 | `GroupTabBadges.boardLatestUpdatedAt` · `shouldShowBoardRecentDot` · `boardSeenStore` |
...
| 匿名群历史 | **本机 SQLite 入库**；leave/enter 不清消息；解散 cascade 删除；**不**走 `chat_sync` 对端补历史 |
