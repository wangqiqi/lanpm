...
**候选下一 Sprint**：会议体验产品化 · chat-perf 剩余债（store-hooks · dm-preview）· 见 `plan.md` 候选表 · [插件开发](./插件开发.md) §12。
...
| **聊天性能（剩余债）** | `ChatView` store 订阅 · `listDmPreviews` · TopBar 门控 · Worker 高亮 | 主序列 **v1.83–v1.87 已交付**；详见 [优化.md](./优化.md) §5 开放项 |
...
| A6 | 离线补同步（项目优先） | 任务/看板/已读等项目数据补同步 | 进行中 · task/chat sync、`task_crdt` |
