...
| 场景 | 动作 |
|------|------|
| **全量审计** | **必须**更新根目录 [`视觉.md`](../../../视觉.md)：校验日期、§10 问题清单、§13 结论；与 `todo.md` §视觉 对齐 |
| **仅门禁** | 可不改文件；口头/聊天汇报 `verify:visual` 结果 + 阶段 A 未完成项 |
| 修 VIS-* | 改代码后重跑 `verify:visual`；全量审计结束后再刷新 `视觉.md` |

文档与代码差异 → 使用 `lanpm-docs-code-audit`，输出 [`代码文档差异.md`](../../../代码文档差异.md)，**不要**写入 `视觉.md`。
