...
| 规划 | Yjs `task_crdt`（**v1.1.0**）· P2P 文件拉取断点续传（**v1.2.0**）· 看板独立标签（目标 **v1.3.0**）· WebRTC · IndexedDB 热缓存（post-RC） |
...
**当前版本：** `1.2.0`（P2P 文件拉取断点续传 + Yjs `task_crdt`；看板独立标签待 **`/release` → v1.3.0**；M0–M7 自动化已闭环）。许可：[AGPL-3.0-or-later](./LICENSE)。真机手验仍延期，见 [验收计划](./docs/06_验收与里程碑计划.md) §2.6。
