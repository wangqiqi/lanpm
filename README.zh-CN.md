...
<p align="center">
  <img src="https://img.shields.io/badge/版本-1.106.3-blue" alt="version" />
...
| **项目管理** | 看板、任务树、甘特、日历、四类依赖、标签、验收清单、Presence、工期健康度。可选**排程插件**（`lanpm.schedule`，可购）：甘特关键路径含 FS/SS/FF/SF，以及按群冻结基线对照。可选**敏捷插件**（`lanpm.agile`，可购）：卡片故事点、列合计、剩余点燃尽 |
...
**当前版本 `1.106.3`** — 免费 `lanpm.backup`（默认开）包单群加密 `.lanpm-bundle`。可购排程：FS/SS/FF/SF 关键路径 + 按群冻结基线。可购敏捷：剩余故事点燃尽。Linux `--full` 抽样 + 安装包冒烟。许可 [AGPL-3.0-or-later](./LICENSE)。
