...
>
> **P0 里程碑 M0–M7 已完成**（`1.0.0-rc.5`）。逐阶段详情见 [archive/20260528_235810_M0-M7完成_plan逐阶段执行计划归档.md](./archive/20260528_235810_M0-M7完成_plan逐阶段执行计划归档.md)  
...
- **??**???????????????????? PR?
- **????**??????? SetupWizard??? Ant Design / ????

---

## 12. ?? i18n?2026-05-29?

> ?? I18N-05~06 ? [todo.md �?? i18n](./todo.md#??-i18n-v10x-??2026-05-29)?

| ?? | ?? | ?? |
|------|------|------|
| ?? | `types.ts` + `locales/*` + `translate` ?? + `navKeys` | ??? |
| ?? | TopBar / BottomNav / GroupView ??? / App ?? | ??? |
| ?? | ??????????????????Setup??? | ??? |
| ?? | `verify:topbar` 232 ? parity??? English ?? | ?????? |

**?????**?`browserLanpmStub` ??? demo ????? IPC ?????
