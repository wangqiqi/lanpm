...
> 依据文档：`docs/01_产品需求文档.md`、`docs/02_技术实现建议.md`、`docs/03_验收与里程碑计划.md`、`docs/04_数据模型与协议草案.md`
>
> 目标周期：20 天（M0-M7）
...
- **需求一致性**：任何实现必须能映射到 `docs/01~03` 的明确条目。
...
| 阶段 | 时间 | 目标 | 主要输出 | 退出标准 |
|------|------|------|----------|----------|
| M0 | Day 1-2 | 项目初始化与首次配置 | Electron 工程骨架、配置向导、用户 ID 生成策略 | 可首次启动并保存身份配置 |
| M1 | Day 3-4 | 主框架与主题能力 | 顶部栏、底部 5 视图导航、主题切换 | 可在 5 视图间稳定切换 |
| M2 | Day 5-7 | 聊天与代码分享 | 成员列表、消息收发、@提及、代码高亮、已读回执 | 聊天链路闭环，通知可触发 |
...
| M6 | Day 16-18 | 网络层强化 | UDP 发现、P2P 加密传输、多设备聚合 | 多节点组网稳定、加密生效 |
...
### 5.1 M0：初始化与身份配置
- 任务清单：
  - 初始化 `src/main` / `src/preload` / `src/renderer` 分层结构。
...
### 5.7 M6：网络层与加密闭环
- 任务清单：
  - UDP 广播节点发现、心跳、离线超时判定。
  - WebRTC P2P 通道建立与重连机制。
  - DH 密钥交换与 AES-GCM 加密收发流程。
  - 同用户多设备在线状态聚合。
- 交付物：
  - 网络层模块（discovery/transport/crypto 三子模块）。
- 验收点：
