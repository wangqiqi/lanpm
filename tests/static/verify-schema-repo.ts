/**
 * AUTO-06 — schema.sql 表清单与 schema.ts EXPECTED_TABLES 一致。
...
const expectedMatch = schemaTs.match(/EXPECTED_TABLES\s*=\s*\[([\s\S]*?)\]\s*as\s*const/)
assert.ok(expectedMatch, 'EXPECTED_TABLES not found in schema.ts')

const expected = [...expectedMatch[1]!.matchAll(/['"](\w+)['"]/g)].map((m) => m[1]!)

for (const t of expected) {
  assert.ok(tables.includes(t), `schema.sql missing table from EXPECTED_TABLES: ${t}`)
}
