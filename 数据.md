...
- 发版前仍执行：`npm run verify:release-gate`（见 `todo.md`）
...
| ✅ 已完成 | 产品定稿 §11 + [todo.md](todo.md) §数据产销闭环 对照表（下表） |
| ⬜ 未执行 | 代码修复与测试 → 以下 `DATA-*` 勾选 |

### 9.1 `数据.md` → `todo.md` 映射（SSOT 对照）
...
- **§4.1–§4.4、§7、§11 可执行项**：均已映射到 `todo.md` 中 `DATA-*`（§4.5 / §1 可接受项除外）。
