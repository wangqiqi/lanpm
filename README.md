...
| **Project mgmt** | Kanban, tree, Gantt, calendar, FS/SS/FF/SF deps, tags, acceptance checklist, presence, schedule health. Optional **schedule plugin** (`lanpm.schedule`, paid): Gantt critical-path highlight. Optional **agile plugin** (`lanpm.agile`, paid): story points on cards + column totals |
| **Collab** | Excalidraw whiteboard + mind maps (Yjs CRDT over P2P), group files, LibreOffice preview, resumable transfers |
| **Org & UX** | Project / functional / anonymous groups, leadership cockpit, light/dark, **zh / en**. Optional **weekly plugin** (`lanpm.weekly`, paid): license-gated weekly/monthly Markdown export |
...
**Current `1.105.0`** — paid SKUs: schedule critical path, agile story points, weekly/monthly report export; anonymous group local history; meeting Pro+ grid. Unreleased: Office light preview (docx/xlsx) when LibreOffice is missing; Gantt task table export (Markdown + CSV). License [AGPL-3.0-or-later](./LICENSE).
