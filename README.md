...
| Roadmap | Yjs `task_crdt`（**v1.1.0**）· P2P file pull resume（**v1.2.0**）· board tags（**v1.3.0**）· task Awareness Presence（**v1.4.0**）· tag filter/palette（目标 **v1.5.0**）· group tag dict sync（目标 **v1.6.0**）· description text caret（目标 **v1.7.0**）· WebRTC · IndexedDB hot cache (post-RC) |
...
**Current:** `1.4.0` — task Awareness Presence; tag filter/palette → **v1.5.0**; group tag dict → **v1.6.0**; description text caret → **v1.7.0** (pending `/release`); M0–M7 closed in automation; true-device hand tests still deferred (see [acceptance plan](./docs/06_验收与里程碑计划.md) §2.6). Licensed under [AGPL-3.0-or-later](./LICENSE).
