...
**Current:** `1.21.0` — B2 member search (`verify:member-search`), plus A1 nudge (1.20), transfer A4 + plugin SPIKE (1.19), platform matrix (1.18), A3/A5/whiteboard realtime (1.17). M0–M7 closed in automation; true-device hand tests still deferred ([acceptance plan](./docs/06_验收与里程碑计划.md) §2.6). License: [AGPL-3.0-or-later](./LICENSE).

**Next (in progress / queue):** Plugin loader + form-js POC in `[Unreleased]` (→ 1.22.0) · plugin marketplace (P2).
