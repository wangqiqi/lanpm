...
- **导航**：项目群底栏跟随全局「导航与视图」；在全局打开甘特/日历后不再被解析层强制隐藏

### Changed

- **看板敏捷条**：当前迭代与新建分开；日期/名称收进弹层，列合计、WIP、燃尽、速度成组换行，不再和原生日期控件挤成一摊
- **甘特 / 树 / 文件**：甘特导出收进下拉、长提示改 `?`；树进度说明同样收进帮助；甘特进度插件条全宽成组；文件筛选与搜索成组换行
- **聊天 Composer**：文件、白板、脑图重新作为工具栏快捷入口，不再收进「更多」
...
- **项目主页**：VitePress 公开文档自洽，不再链到仓库内部 `docs/`；**默认语言为中文**（`/`），英文在 `/en/`；旧 `/zh/` 跳转到根路径（`verify:docs-links` 防回退）
- **发现向导**：第二步无群/无成员/无申请时显示引导并提供「返回连通」（`TASK-DWIZ-01`）
- **发现**：入群申请展示群名称，不再露出内部 `groupId`（`TASK-DWIZ-02`）
- **发现向导**：英文长文案下 Steps / CTA / 配对区可折行收缩（`TASK-DWIZ-03`）
- **发现 E2E**：向导两步切换；Coachmark 遮罩不再挡住配对按钮（`TASK-DWIZ-04`）
- **发现**：`docs/04` 对齐两步向导加固；`verify:discover` 守卫空步引导与群名解析（`TASK-DWIZ-05`）
...
- **双机验证**：`npm run userdata:wipe` · `npm run dev:deploy-test -- <peer>:43124`（清库 + 连种子 + TCP 探测后启动）；`docs/05` §6.1 · README（`TASK-DUAL-A04`）
- **协作**：`.cursorGrowth/plan.md` 纳入版本库（其余 Growth 仍本地忽略）
- **插件**：启用 paid 插件且无许可证时追加说明 toast（`TASK-DUAL-04d`）

### Fixed

- **单实例**：主进程在分流 `userData` 前加机器级锁（`~/.cache/LanPM/instance.pid`）+ Electron `requestSingleInstanceLock`；`LANPM_ALLOW_MULTI_INSTANCE=1` / E2E / 视觉截图才允许多开；`npm run lanpm:stop` · `dev:deploy-test` 启动前自动停旧进程
- **发现成员列表**（`TASK-DUAL-04a`）：中继不再给幽灵 peer 刷在线；`tcpPeers` 按 TTL 剔除无 UDP/无链路的旧 userId；同一 LAN IP 只展示一个用户（优先已握手）
...
- **导航 UX P2-2（SPRINT-UX-87）**：项目群无覆盖时解析层默认折叠甘特/日历；看板可关闭 hint 指向「导航与视图」（`TASK-8701`..`TASK-8705`）
...
- **测试文档**：`docs/05` 收口回归命令、资产路径、底栏 vs 协作抽屉、`dev:web`/5173 playbook、IBUS 非阻断、§9 待补测表；站点 quick-start 补充导航偏好说明（`TASK-7501` · `TASK-7504`）
...
- **敏捷速度口径**：按迭代汇总 `done` 估点（未估点不计）（`TASK-7101`）
- **敏捷速度 IPC**：`getAgileVelocity` 付费闸 + 浏览器 stub（`TASK-7102`）
- **看板速度图**：工具栏柱状图；无完成点空态（`TASK-7103`）
- **敏捷速度口径**：`docs/06` §6.3 · README 记 SPRINT-71；**carry-over 仍后置**（`TASK-7104`）
...
- **看板迭代过滤**：工具栏建/选迭代；卡片挂入；未挂任务不进当前迭代看板（`TASK-7003`）
- **敏捷迭代口径**：`docs/06` §6.3 · README 记 SPRINT-70；速度图 / carry-over 仍后置（`TASK-7004`）

## [1.106.8] - 2026-08-21

### Fixed

- **聊天协作工具栏图标**：文件/白板/脑图与底栏同一套（文件夹 · 高亮笔 · 节点图），不再用左栏布局和任务树图标
- **发送键朝向**：聊天与 AI 助手 `SendOutlined` 旋转朝上（送入上方会话），避免默认朝右飞出屏幕

## [1.106.7] - 2026-08-21

### Changed

- **文档滞后对齐**：README 功能表补上排程指派重叠与敏捷列 WIP；`docs/06` 去掉双「§6」、开源债务改 **§7**、候选队列改 **§8**；删未引用的 `whiteboard.placeholder` 死键
...
- **看板 WIP 提示**：授权后工具栏改上限；列超限只提示不拦拖拽；无许可仍 CTA（`TASK-6902`）
- **敏捷 SKU 守卫**：`verify:agile-sku` 锁住 WIP 表、许可闸、工具栏、列提示、禁止拦拖拽（`TASK-6903`）
- **敏捷 WIP 口径**：`docs/06` §6.3 · README 记 SPRINT-69；迭代容器仍后置（`TASK-6904`）

## [1.106.5] - 2026-08-21

### Added

- **指派重叠检测**：同负责人且显式起止日相交（含同日相接）；无日期/未指派不参与（`TASK-6801`）
- **甘特重叠提示**：授权后工具栏计数 + 条色（紫）；关键路径仍优先；无许可仍 CTA（`TASK-6802`）
- **排程 SKU 守卫**：`verify:schedule-sku` 锁住指派重叠纯函数、工具栏、条色、禁止默认窗（`TASK-6803`）
- **排程重叠口径**：`docs/06` §6.3 · README 记 SPRINT-68；容量 leveling 仍后置（`TASK-6804`）
...
- **月报独立模板**：本月里程碑 / 进度 / 风险汇总，不再周报换标题（`TASK-6402`）
- **周报 SKU 守卫**：`verify:weekly-sku` 锁住下周计划与月报独立模板；评估仍免费（`TASK-6403`）
- **周报口径**：`docs/06` · `docs/01` §12.2 · README 记 SPRINT-64（`TASK-6404`）
...
- **甘特关键路径**：付费排程按 FS/SS/FF/SF 依赖边算最长工期链（仍非日历 CPM）（`TASK-6201`）
- **关键路径文案**：空链提示不再写死「仅 FS」（`TASK-6202`）
- **排程 SKU 守卫**：`verify:schedule-sku` 锁住四类依赖边进关键路径（`TASK-6203`）
- **排程口径**：`docs/06` §6.3 标四类依赖关键路径已交付；资源平衡 / 基线仍后置（`TASK-6204`）
