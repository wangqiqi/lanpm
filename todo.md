...
> **来源**：2026-05-29 暗色六页手验审阅（Chat · Board · Tree · Gantt · Files · Cockpit）；发版 P2 阻断仍含 V-14b / 文档。

### 代码修复（暗色手验暴露）

| 优先级 | ID | 任务 | 涉及 | 验收 | 状态 |
|--------|-----|------|------|------|------|
| **P1** | **VIS-FIX-01** | 甘特暗色时间轴表头（月份/W 行）勿呈白/浅灰条 | `gantt.module.css` · `gantt-task-react` 覆盖 | 暗色下表头与 `chartWrap` 同系 surface，无亮色嵌入感 | ✅ |
| **P2** | **VIS-FIX-02** | 驾驶舱 1280×720 无横向滚动条 | `CockpitView.module.css` 等 | 全宽布局下 `document` 无多余 `overflow-x` | ✅ |
| **P3** | **VIS-FIX-03** | 看板优先级等 Ant 红标签改 `--lanpm-danger` | `BoardView` / Tag 样式 | 与语义危险色一致 | ✅ |
| **P3** | **VIS-FIX-04** | 甘特任务条暗色对比（条 fill 略提亮或依赖今日线） | `gantt.module.css` | 暗底上条与网格可区分 | ✅ |

### V-14b 手验（截图 + 人工）

| 优先级 | ID | 任务 | 说明 | 状态 |
|--------|-----|------|------|------|
| **P2** | **V-14b-D** | 暗色七页截图归档 | 已有 6 页；补 **Setup**；本地 `dark_{page}.png`，勿提交 | [ ] |
| **P2** | **V-14b-L** | 亮色七页截图 | `light_{page}.png` 全套 | [ ] |
| **P2** | **V-14b-SEM** | 暗色语义色观感 | 红/绿/黄（看板优先级、任务树进度、顶栏状态）— 审阅已通过，勾选前再扫一眼 | [ ] |
| **P2** | **V-14b-HOV** | Ant 按钮 vs `RegionButton` / `ViewSegment` hover 并排 | 顶栏、底栏、文件/甘特分段 | [ ] |
| **P2** | **V-14b-GNT** | 甘特修复后复截 | 依赖 **VIS-FIX-01** ✅ — 请本地复截 `dark_gantt.png` | [ ] |

### 发版 / 文档（与上表并行）

| 优先级 | ID | 任务 | 命令 / 说明 | 状态 |
|--------|-----|------|-------------|------|
| **P2** | **DOC-02** | 全量文档审计 → 更新 `代码文档差异.md` §2 | §1 机读 `--strict` ✅；§2 PRD/06 抽检仍待 Agent | [ ] |
| **P3** | **AUTO-18** | CI 三平台矩阵 | — | [ ] |
| **P3** | **AUTO-20** | 截图回归（Playwright/Electron，减轻后续 V-14b） | 依赖 AUTO-19 ✅ | [ ] |

**推荐顺序**：VIS-FIX-01 → VIS-FIX-02 → V-14b-GNT → V-14b-D/L → V-14b-HOV/SEM → VIS-FIX-03/04 → DOC-02 → AUTO-18/20。
...
| **P3** | 自动化 | AUTO-18 / AUTO-20（**AUTO-19** ✅） | [ ] |
| **手验** | 全项 | §手验 表 | [ ] |
