...
- [x] `M6-01` [P0] UDP 广播发现（端口 43123、心跳/超时参数按 `docs/02`）
- [x] `M6-02` [P0] WebRTC DataChannel 建链与重连（DoD：LAN TCP 可靠数据通道 + 指数退避重连）
- [x] `M6-03` [P0] DH + AES-GCM 加密收发
- [x] `M6-04` [P0] 用真实网络替换 Stub（保留 Stub；`LANPM_NETWORK=stub|real`）
- [x] `M6-05` [P0] 同用户多设备在线聚合复验（`verify:presence` + 真网心跳）
- [x] `M6-06` [P0] 真网双机/多机联调（DoD：`verify:m6` 加密回环 + `docs/06` §4 手验清单）

---

## M7 测试与发布（Day 19-20）

- [x] `M7-01` [P0] `docs/03` §16.1~16.12 全量验收打勾（DoD：`docs/08_M7_RC验收清单.md` + `verify:m7-acceptance`）
- [x] `M7-02` [P0] 性能测试（DoD：`verify:m7-perf` 自动化冒烟；冷启动/内存见 docs/06 §3 手测模板）
- [x] `M7-03` [P0] 稳定性测试（DoD：`verify:m7-stability` SQLite 崩溃恢复）
- [x] `M7-04` [P0] 缺陷清零（P0/P1）（DoD：`verify:m7` 全绿；已知限制记入 docs/08）
- [x] `M7-05` [P0] RC 打版 + `CHANGELOG.md` 更新（`1.0.0-rc.1`）
...
- [x] `M6-01`~`M6-06` 真实网络层（`verify:m6`；Stub 保留 `LANPM_NETWORK=stub`）
- [x] `M7-01`~`M7-05` RC 验收与发布（`verify:m7` · `1.0.0-rc.1`）
